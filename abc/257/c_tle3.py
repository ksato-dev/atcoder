...
num_m_pc_list = len(merged_pc_list)
s_m_t_list = [0] * (num_m_pc_list + 1)
for i in range(1, num_m_pc_list):
    s_m_t_list.append(m_t_list[i-1] + s_m_t_list[i-1])
...
for i in range(num_m_pc_list):
    ...
    sum_person = s_m_t_list[num_m_pc_list] - s_m_t_list[i]
    sum_child = len(m_t_list[:i]) - (s_m_t_list[i] - s_m_t_list[0])
    if i < len(merged_pc_list) - 1:
        ...
        if next_weight - weight >= 2:
            ...
            sum_person = s_m_t_list[num_m_pc_list] - s_m_t_list[i+1]
            sum_child = len(m_t_list[:i+1]) - (s_m_t_list[i+1] - s_m_t_list[0])
